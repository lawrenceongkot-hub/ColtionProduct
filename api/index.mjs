  ...
  try {
    ...
    if (p.startsWith('/api/payments/')) {
      try {
        ...
        console.error('[Vercel Proxy] Payment proxy error:', e?.message || e);
        return res.status(502).json({ error: 'Payment gateway temporarily unavailable', provider: 'EC2-proxy' });
      }
    }
    ...
    if (m === 'GET' && p === '/api/admin/dashboard') {
      try {
        ...
        // ISSUE 2: Net Revenue = deposits - released amount only.
        // The fee is ALREADY retained because we subtract netAmount (released), not requestedAmount.
        // Adding fee again would double-count it.
